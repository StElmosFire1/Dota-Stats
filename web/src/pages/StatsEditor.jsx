          ...
          <table style={{ borderCollapse: 'collapse', width: '100%', fontSize: '0.8rem' }}>
            ...
            <tbody>
              {players.map((pl, idx) => {
                ...
                const teamColor = isRadiant ? 'rgba(76,175,80,0.08)' : 'rgba(244,67,54,0.08)';
                return (
                  <tr
                    key={pl.slot}
                    style={{
                      background: idx % 2 === 0 ? teamColor : 'transparent',
                  ...
                  >
                    <td style={{ ...cellStyle, padding: '4px 8px', position: 'sticky', left: 0, background: idx % 2 === 0 ? (isRadiant ? '#0e1c0f' : '#1c0e0e') : '#111827' }}>
                      <span style={{ color: isRadiant ? '#4caf50' : '#f44336', fontSize: '0.7rem' }}>
                        {isRadiant ? 'R' : 'D'}
                      </span>{' '}
                      <span style={{ color: '#e0e0e0' }}>{pl.persona_name || `Slot ${pl.slot}`}</span>
                    </td>
                    <td style={{ ...cellStyle, padding: '4px 6px', color: '#aaa', whiteSpace: 'nowrap' }}>
                      {pl.hero_name || '—'}
                    </td>
                    {fields.map(f => (
                      <PlayerCell
                        ...
                        player={pl}
                        field={f}
                        onChange={(field, val) => updatePlayer(pl.slot, field, val)}
                      />
                    ))}
                  </tr>
                );
