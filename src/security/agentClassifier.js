// Task #492 — Runtime UA classifier middleware.
//
// What it does on every request:
//   1. Classifies the User-Agent header against agentUaList.classifyUa().
//   2. Sets X-Robots-Tag: noai, noimageai on the response (cheap; harmless
//      to humans, picked up by AI crawlers that honour it).
...
// so that one misbehaving family can't take down others sharing the IP, and
// so that distinct families sharing an IP each get their own quota.
const agentLimiter = rateLimit({
  ...
  // merely logged traffic — see Task #492 acceptance.
...
function classifierMiddleware(req, res, next) {
  ...
  return agentLimiter(req, res, (err) => {
    ...
    // If express-rate-limit already responded (429), it won't call next;
    // when it does call next, we tag the buffer entry as throttled if the
    // limiter has marked the request.
    if (res.headersSent) return; // 429 already returned by limiter
    next();
  });
}

// Report builder — used by the superuser admin route. Aggregates the
// ring buffer over a window (defaults to 7 days) by (family, decision).
function buildReport({ windowMs = 7 * 24 * 60 * 60 * 1000 } = {}) {
  ...
  // Recent samples (newest first, capped) so the admin can eyeball
