...
class ReplayParser {
  ...
  _aggregateStats(events) {
    ...
    for (const e of events) {
      ...
      // Match on inflictor or key containing smoke_of_deceit regardless of event type,
      // since the exact combat log type varies by replay version.
      if ((e.inflictor === 'item_smoke_of_deceit' || e.key === 'item_smoke_of_deceit' ||
      ...
      // Match on inflictor or key regardless of event type (same reason as smoke).
      if ((e.inflictor === 'item_dust' || e.key === 'item_dust' ||
