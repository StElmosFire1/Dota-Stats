...
**Pro-paywall regression gate (Task #299):** after the community frontend build, `community-edition/deploy.sh` runs `bash scripts/check-community-paywall.sh`, which `grep -RFl`s the built `community-edition/web/dist/` for forbidden full-edition-only tokens (`PaywallCard`, `useProStatus`, `Pro Tier`, `Inhouse Stats Pro`) and aborts the deploy if any are found, naming the offending chunk(s). The same gate runs in `scripts/post-merge.sh` (which now also builds `community-edition/web/` alongside the top-level `web/`), so a regression fails the GitHub push **before** it ever reaches the prod host. This is the belt-and-braces backstop for Task #298's deploy-script split — if someone copy-pastes a `PaywallCard` import into `community-edition/web/src/` in future, the gate catches it.
