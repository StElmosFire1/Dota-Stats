...
- `FULL_SITE_LOCKDOWN` — Task #493 owner-only login gate for the full edition (`oceinhouse.gg`). **Unset** (default) means no change in behavior. Set to `1` to hide every page, asset, and API behind a minimal password gate: unauthenticated HTML navigations get an inline sign-in page (no app HTML, no meta description, no OG tags); non-HTML requests get a 401 with an empty body. Allowlist (always works regardless of the flag): `/api/stripe/webhook`, `/api/admin/superuser-login`, `/auth/steam`, `/auth/steam/return`, `/api/auth/complete`, `/robots.txt`, `/favicon.ico`. Successful superuser login (session cookie OR `x-superuser-key` header) bypasses the gate. To remove the gate at launch, unset the env var and restart — no code change required. Community edition (`dota.stats.corvidaeinc.com`) is untouched.
