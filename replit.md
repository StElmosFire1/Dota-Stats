...
- `SUPERUSER_STEAM_IDS` — Task #749 superuser↔Steam binding (full edition only; community untouched). Comma/space/newline-separated list of **Steam 32-bit account ids** (the same value stored on `req.session.accountId`) that are permitted to unlock superuser via the interactive password sign-in (`POST /api/admin/superuser-login`, used by both the AdminPanel "Superuser Login" modal and the `FULL_SITE_LOCKDOWN` gate page). When the var is **unset/empty** the binding is **disabled** — the password alone elevates exactly as before; this is the safe default that can NEVER lock the owner out (there is no stored owner Steam identity to fall back to, so an empty allow-list deliberately means "no binding"). When **set**, superuser elevation requires ALL of: (1) correct `SUPERUSER_PASSWORD`, (2) an authenticated Steam session (`req.session.accountId` present), and (3) that account id on this list — so a leaked password alone is useless. Distinct failures: wrong password → 401 "Invalid password"; right password but not signed in / not allow-listed → 403 with a plain-English message (surfaced in the login modal and on the gate page, which also shows a "Sign in with Steam first" link). **The machine-key header path is deliberately NOT bound** — `x-superuser-key: <SUPERUSER_PASSWORD>` (scripts, deploy hooks, `getEffectiveRole`/`requireSuperuser`/lockdown middleware) keeps working unchanged so server-to-server automation is unaffected. To enable: find your account id (the numeric id in your `/profile` URL / session), set `SUPERUSER_STEAM_IDS=<your id>` (add more ids comma-separated for co-owners), then restart.
