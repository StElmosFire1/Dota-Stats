...
- **Source-scan pass** (Task #303): `grep -RFl`s `community-edition/src/` and `community-edition/web/src/` for ten forbidden tokens — `PaywallCard`, `useProStatus`, `useProMembers`, `isProMember`, `paywall: true`, `"paywall": true`, `Pro membership`, `requires Pro`, `Pro Tier`, `Inhouse Stats Pro`. `community-edition/SETUP.md` and `community-edition/src/data/patchNotes.js` are explicitly allow-listed so we can still describe historical paywalls in prose.
