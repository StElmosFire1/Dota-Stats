...
- **MMR Display Baseline (Season 10):** Display MMR formula is `round((μ − 3σ) × 100) + 5000`. A fresh player at μ=25/σ=8.333 reads as exactly **5000** on every surface (leaderboard, profile, badges, embeds, exports). Underlying TrueSkill maths are unchanged.
- **8-Tier MMR Ladder + `season_tiers`:** New `MmrBadge` component (Tier I–VIII) renders in PlayerProfile/Leaderboard gated on `new_rank_theme`. MMR floors at 0 / 2000 / 3000 / 4000 / 5000 / 6000 / 7000 / 8000; Tier V is the default. Per-season tier names/floors live in the `season_tiers` table and are editable via the Admin Panel "Season Tiers" panel — superusers can seed the default 8-tier ladder, edit names/floors, and one-click "place all players by MMR" (writes to `season_tier_players`). Tournaments carry an optional `tier_number` for tier-scoped events.
- **Per-Tournament Stripe Buy-ins (`tournament_self_signup`):** `tournaments` gained `entry_fee` / `signup_opens_at` / `signup_closes_at`; new `tournament_entries` table holds per-player signups. Self-signup runs through Stripe Checkout (or marks the player entered for free events); the prize pool auto-grows from collected fees. Eligibility is enforced server-side against the tournament's `tier_number`.
- **MVP-per-Match Badges (`mvp_match_badges`):** `getMatch` returns `mvp_account_id` / `mvp_vote_count`; `recentMatches` adds an `is_mvp` boolean via correlated subquery on `match_mvp_votes`. ⭐ rendered in MatchDetail scoreboard rows and PlayerProfile match history.
- **Profile Chart V2 (`profile_chart_v2`):** New `/player/:id/match-stats-history` endpoint + `ProfileChartV2` widget on PlayerProfile shows 5-game rolling K/D/A and GPM over the last ~100 matches. Visible only to the logged-in player viewing their own profile.
- **Welcome Modal (`welcome_modal_s10`):** One-shot launch modal mounted in `App.jsx`, dismissed per-browser via `localStorage` key `welcomeModalS10Dismissed_v1`.
- **Home Page Join CTA (`home_join_button`):** Prominent CTA on the home page routing to `/join`.
- **Feature Flags + Season 10 Launch:** Three-state toggle (`off` / `preview` / `on`) backed by the `feature_flags` table. Off = hidden, preview = visible only to logged-in superusers, on = live for all. Resolved at request time — `GET /api/feature-flags` returns a `{ key: bool }` map for the caller, picked up by `FeatureFlagsProvider` and consumed via `useFeatureFlag(key)`. Admin Panel "Feature Flags" section lets superusers list/edit flags and includes a "Launch Season 10 Now" button (with confirmation) that triggers the same code path as the launch cron. Cron is scheduled in `src/discord/bot.js` for **Friday 1 May 2026 9:00am AEST** (`Australia/Sydney`); it calls `db.executeSeason10Launch()` (idempotent — short-circuits via `season_10_launched_at` site setting), bulk-flips every preview flag to on, force-enables `home_launch_banner`, and posts a Discord launch embed via `bot.announceSeason10Launch()`. Discord post is gated on `ANNOUNCE_PATCH_NOTES === 'true'`. Front-end launch banner lives at the top of the home page, dismissible per-browser.
...
- **Automated Tests (`npm test`):** Node's built-in test runner covers the V3 rating engine math edges in `tests/v3RatingEngine.test.js`: sigma floor enforcement, modifier clamps at 0.80/1.20, modifier=1.0 fallback for lobby-only matches, z-score → modifier mapping, and the defensive cross-team collision guard in `computeSeasonTrueSkillV3`. The guard helpers (`_v3PerfScore`, `_v3ScoresToModifiers`, `_v3HasCrossTeamCollision`) are exported from `src/db/index.js` for testability.
