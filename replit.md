...
The pass also handles the `<button>{expr}</button>` shape (Task #183). Originally any `{…}` JSX expression caused the element to be skipped because `{label}` *might* be a visible text label; that left a hole where `<button onClick={x}>{icon}</button>` (an icon component reference) silently passed. The classifier now distinguishes:
...
Synthetic test coverage for both shapes lives in `tests/checkA11y.test.js`. Same `deploy.sh` / `scripts/post-merge.sh` hard gate.
