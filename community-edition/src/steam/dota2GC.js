...
const { EDOTAGCMsg, ESOMsg, CSODOTALobby, CMsgSOCacheSubscribed, CMsgSOSingleObject, CMsgSOMultipleObjects, CMsgClientWelcome } = require('dota2-user/protobufs');
    ...
    try {
      ...
      for (const obj of objects) {
        ...
        console.log(`[Dota2 GC] SO CacheSubscribed object: typeId=${typeId}` +
          (typeId === LOBBY_TYPE_ID ? ' (Lobby)' : typeId === 2003 ? ' (Party-2003)' : ''));
  ...
  _handleSOCreate(payload) {
    try {
      ...
      console.log(`[Dota2 GC] SO Create: typeId=${msg.typeId}, dataLen=${msg.objectData ? msg.objectData.length : 0}` +
        (msg.typeId === LOBBY_TYPE_ID ? ' (Lobby)' : msg.typeId === 2003 ? ' (Party-2003)' : ''));
