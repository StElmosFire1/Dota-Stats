...
class DiscordBot {
  ...
  async _cmdPerfBackfill(msg, args) {
    ...
    try {
      ...
      await msg.reply(`PERF backfill complete. Total: ${r.total}, processed: ${r.processed}, ok: ${r.ok}, failed: ${r.failed}.`);
    } catch (err) {
  ...
  async _handleRegistrationReply(msg) {
    ...
    if (input.toLowerCase() === 'skip' || input.toLowerCase() === 'cancel') {
      ...
      await msg.reply(
        ...
        `You can register any time with \`!register <steam_id>\` in the Discord server. See you at the inhouse! 🎮`
      );
      return;
    }
    ...
    try {
      ...
      console.log(`[Registration] Registered ${msg.author.username} (${msg.author.id}) via DM — Steam64: ${steamId64} (format: ${format})`);
    } catch (err) {
  ...
  async _cmdAnalyze(msg, args) {
    return msg.reply('AI analysis is not available in the community edition.');
  }
  ...
  async _sendReportCardDMs(matchStats) {
    ...
    for (const player of matchStats.players) {
      try {
        ...
        const allowed = await (db.isNotificationEnabled
          ? db.isNotificationEnabled(reg.account_id_32, 'post_match_dm').catch(() => true)
  ...
  async start() {
    ...
    this.client.once('ready', () => {
      ...
      // timeline_v1 baselines stay current as the patch meta evolves.
      cron.schedule('0 3 * * 1', () => {
      ...
      // older than ~10 days, so a freshly deployed bot does not have to wait a
      // full week before timeline_v1 becomes usable.
      setTimeout(() => {
        (async () => {
          try {
